...
async function runUploadSmokeTest(window) {
  ...
  const result = await window.webContents.executeJavaScript(`(async () => {
    ...
    window.confirm = () => true;
    const staleSmokeFolders = [...document.querySelectorAll(".library-folder-tile")].filter(tile => /^mathmargin-smoke-.*-folder-renamed$/.test(tile.querySelector("strong")?.textContent?.trim() ?? ""));
    for (const tile of staleSmokeFolders) {
      tile.click(); await new Promise(resolve => setTimeout(resolve, 100));
      document.querySelector(".delete-folder-command")?.click(); await new Promise(resolve => setTimeout(resolve, 100));
      document.querySelector(".confirm-delete-folder-button")?.click(); await new Promise(resolve => setTimeout(resolve, 150));
    }
    ...
    const folderOpenStartedAt = Date.now();
    while (document.querySelector(".books-section h2")?.textContent !== smokeFolderName && Date.now() - folderOpenStartedAt < 3000) await new Promise(resolve => setTimeout(resolve, 100));
    ...
    while (Date.now() - startedAt < 15000) {
      ...
      if (smokeCards().length > initialSmokeCount) {
        ...
        if ([...document.querySelectorAll(".folder-nav > button")].some(button => button.textContent?.includes("All PDFs"))) return { ok: false, folderCreated: true, textbookStructureSaved: true, error: "The All PDFs section is still visible." };
        const unfiledViewStartedAt = Date.now();
        while ((!smokeCards().length || !smokeFolderTiles().length) && Date.now() - unfiledViewStartedAt < 5000) await new Promise(resolve => setTimeout(resolve, 100));
        ...
        while (smokeCards().length && Date.now() - selectedMoveStartedAt < 5000) await new Promise(resolve => setTimeout(resolve, 100));
        if (smokeCards().length) return { ok: false, folderCreated: true, textbookStructureSaved: true, error: "The selected PDF remained visible in Unfiled after moving to a folder." };
        destinationFolder.click();
        const selectedFolderStartedAt = Date.now();
        while ((!smokeCards().length || document.querySelector(".books-section h2")?.textContent !== smokeFolderName) && Date.now() - selectedFolderStartedAt < 5000) await new Promise(resolve => setTimeout(resolve, 100));
        draggedCard = smokeCards()[0];
        if (!draggedCard || draggedCard.querySelector(".explorer-location")?.textContent !== smokeFolderName) return { ok: false, folderCreated: true, textbookStructureSaved: true, error: "The selected PDF was not present in the chosen destination folder." };
        if (!draggedCard.classList.contains("is-selected")) draggedCard.querySelector(".card-selection-toggle")?.click();
        await new Promise(resolve => setTimeout(resolve, 100));
        ...
        while (smokeCards().length && Date.now() - unfiledMoveStartedAt < 5000) await new Promise(resolve => setTimeout(resolve, 100));
        if (smokeCards().length) return { ok: false, folderCreated: true, textbookStructureSaved: true, selectedPdfMoved: true, error: "The moved PDF remained visible in its old folder." };
        document.querySelector(".explorer-up-button")?.click();
        const unfiledHomeStartedAt = Date.now();
        while ((!smokeCards().length || document.querySelector(".books-section h2")?.textContent !== "Unfiled PDFs") && Date.now() - unfiledHomeStartedAt < 5000) await new Promise(resolve => setTimeout(resolve, 100));
        if (!smokeCards().length || document.querySelector(".books-section h2")?.textContent !== "Unfiled PDFs") return { ok: false, folderCreated: true, textbookStructureSaved: true, selectedPdfMoved: true, error: "Unfiled was not the root page after moving the PDF back." };
        draggedCard = smokeCards()[0];
        const refreshedDestinationFolder = smokeFolderTiles()[0];
        if (!refreshedDestinationFolder) return { ok: false, folderCreated: true, textbookStructureSaved: true, selectedPdfMoved: true, error: "Root folders were not visible beside Unfiled PDFs." };
        ...
        refreshedDestinationFolder.dispatchEvent(new DragEvent("dragover", { bubbles: true, cancelable: true, dataTransfer: moveTransfer }));
        refreshedDestinationFolder.dispatchEvent(new DragEvent("drop", { bubbles: true, cancelable: true, dataTransfer: moveTransfer }));
        const moveStartedAt = Date.now();
        while (smokeCards().length && Date.now() - moveStartedAt < 5000) await new Promise(resolve => setTimeout(resolve, 100));
        if (smokeCards().length) return { ok: false, folderCreated: true, folderIconVisible: true, textbookStructureSaved: true, error: "Dragging the PDF onto the folder did not remove it from Unfiled." };
        smokeFolderTiles()[0].click();
        ...
        const reverseLinkedItem = document.querySelector(".annotation-linked-item > button:first-child");
        if (!reverseLinkedItem?.textContent?.includes("A ↔ B") || !document.querySelector(".annotation-links-heading")?.textContent?.includes("two-way")) return { ok: false, uploaded: true, rendered: true, areaMoved: true, boxOptions: true, annotationsLinked: true, error: "Selecting annotation B did not show its reverse connection back to annotation A." };
        reverseLinkedItem.click();
        const reverseLinkOpenedAt = Date.now();
        while (!document.querySelector(".annotation-mark.area.selected") && Date.now() - reverseLinkOpenedAt < 5000) await new Promise(resolve => setTimeout(resolve, 100));
        if (!document.querySelector(".annotation-mark.area.selected")) return { ok: false, uploaded: true, rendered: true, areaMoved: true, boxOptions: true, annotationsLinked: true, error: "The reverse B-to-A annotation link did not open annotation A." };
        ...
        const importFile = new File([bytes], "imported-" + smokeToken + "-one.pdf", { type: "application/pdf" });
        const secondImportFile = new File([bytes], "imported-" + smokeToken + "-two.pdf", { type: "application/pdf" });
        Object.defineProperty(importFile, "webkitRelativePath", { configurable: true, value: importFolderName + "/nested/imported-" + smokeToken + "-one.pdf" });
        Object.defineProperty(secondImportFile, "webkitRelativePath", { configurable: true, value: importFolderName + "/nested/imported-" + smokeToken + "-two.pdf" });
        const importTransfer = new DataTransfer(); importTransfer.items.add(importFile); importTransfer.items.add(secondImportFile);
        ...
        const importedCardsAtRoot = [...document.querySelectorAll(".book-card")].filter(card => card.textContent?.includes("imported-" + smokeToken));
        const nestedFolderTile = [...document.querySelectorAll(".library-folder-tile")].find(tile => tile.querySelector("strong")?.textContent?.trim() === "nested");
        if (importedCardsAtRoot.length || !nestedFolderTile || document.querySelector(".books-section h2")?.textContent !== importFolderName) return { ok: false, folderRenamed: true, folderDeleted: true, error: "The imported PDFs were flattened instead of remaining inside their nested folder." };
        nestedFolderTile.click();
        const nestedFolderStartedAt = Date.now();
        while (document.querySelector(".books-section h2")?.textContent !== "nested" && Date.now() - nestedFolderStartedAt < 5000) await new Promise(resolve => setTimeout(resolve, 100));
        const importedCards = [...document.querySelectorAll(".book-card")].filter(card => card.textContent?.includes("imported-" + smokeToken));
        if (importedCards.length !== 2 || document.querySelector(".books-section h2")?.textContent !== "nested" || importedCards.some(card => !card.querySelector(".document-type-badge.textbook-structure"))) return { ok: false, folderRenamed: true, folderDeleted: true, subfolderHierarchy: true, error: "The imported nested folder did not contain both textbook-enabled PDFs." };
        importedCards.forEach(card => card.querySelector(".card-selection-toggle")?.click());
        await new Promise(resolve => setTimeout(resolve, 150));
        if (!document.querySelector(".explorer-status-bar")?.textContent?.includes("2 items selected") || document.querySelectorAll(".book-card.is-selected").length !== 2) return { ok: false, folderRenamed: true, folderDeleted: true, error: "Two PDFs could not be selected together." };
        const bulkTextbookButton = document.querySelector(".explorer-command-bar .textbook-treatment-command");
        if (!bulkTextbookButton?.textContent?.includes("Disable textbook structure")) return { ok: false, folderRenamed: true, folderDeleted: true, multiSelection: true, error: "The bulk textbook action did not reflect the selected PDFs." };
        bulkTextbookButton.click();
        const bulkActionStartedAt = Date.now();
        while ([...document.querySelectorAll(".book-card")].some(card => card.querySelector(".document-type-badge.textbook-structure")) && Date.now() - bulkActionStartedAt < 5000) await new Promise(resolve => setTimeout(resolve, 100));
        if ([...document.querySelectorAll(".book-card")].some(card => card.querySelector(".document-type-badge.textbook-structure"))) return { ok: false, folderRenamed: true, folderDeleted: true, multiSelection: true, error: "The bulk textbook action did not update every selected PDF." };
        document.querySelector(".explorer-up-button")?.click();
        const importedRootStartedAt = Date.now();
        while (document.querySelector(".books-section h2")?.textContent !== importFolderName && Date.now() - importedRootStartedAt < 5000) await new Promise(resolve => setTimeout(resolve, 100));
        if (document.querySelector(".books-section h2")?.textContent !== importFolderName) return { ok: false, folderRenamed: true, folderDeleted: true, multiSelection: true, bulkTextbookAction: true, error: "The Explorer up button did not return from the nested folder to its imported parent." };
        document.querySelector(".delete-folder-command")?.click(); await new Promise(resolve => setTimeout(resolve, 100));
        const cascadeDialog = document.querySelector(".folder-delete-dialog");
        if (!cascadeDialog?.textContent?.includes("1 subfolder") || !cascadeDialog.textContent?.includes("2 PDFs") || !cascadeDialog.textContent?.includes("removed from your library")) return { ok: false, folderRenamed: true, folderDeleted: true, multiSelection: true, bulkTextbookAction: true, error: "Folder deletion did not warn that nested folders and PDFs would be deleted." };
        document.querySelector(".confirm-delete-folder-button")?.click();
