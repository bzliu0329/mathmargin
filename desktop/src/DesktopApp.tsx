...
function DesktopLibrary({ onOpen, onRename, onDelete }: { onOpen: (document: LocalDocument) => void; onRename: (id: string, title: string) => void; onDelete: (id: string) => void }) {
  ...
  async function discardFolder(folder: LibraryFolder) {
    ...
    try {
      ...
      if (libraryView.startsWith("folder:") && removedFolderIds.has(libraryView.slice(7))) setLibraryView(folder.parentFolderId ? `folder:${folder.parentFolderId}` : "all");
  ...
  const parentLibraryView = selectedFolder?.parentFolderId ? `folder:${selectedFolder.parentFolderId}` : "all";
  const visibleFolders = (libraryView === "unfiled" ? [] : folders.filter((folder) => (folder.parentFolderId ?? null) === currentFolderId)).filter((folder) => !query || folder.name.toLowerCase().includes(query));
  ...
  return <main className="library-shell desktop-library">
    ...
    <section className="explorer-command-bar" aria-label="Library commands">
      ...
      <label className={`bulk-move-control ${selectedDocuments.length ? "enabled" : ""}`}><span>Move to</span><select aria-label="Move selected PDFs to folder" value="" disabled={!selectedDocuments.length} onChange={(event) => { const folderId = event.target.value === "__unfiled__" ? "" : event.target.value; if (event.target.value) void moveDocuments(selectedDocuments, folderId); }}><option value="">Move to…</option><option value="__unfiled__">Unfiled</option>{folders.map((folder) => <option value={folder.id} key={folder.id}>{folderPathLabel(folders, folder)}</option>)}</select></label>
