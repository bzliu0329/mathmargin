...
function DesktopLibrary({ onOpen, onRename, onDelete }: { onOpen: (document: LocalDocument) => void; onRename: (id: string, title: string) => void; onDelete: (id: string) => void }) {
  ...
  function chooseFolderImport(files?: FileList | null) {
    ...
    setPendingFolderImport({ name, files: selected }); setFolderImportTreatAsTextbook(false); setFolderImportProgress(0); setError("");
  }
    ...
    try {
      ...
      for (let index = 0; index < pdfFiles.length; index++) {
        ...
        try {
          ...
          added.push({ id: crypto.randomUUID(), title: relativeName.replace(/\.pdf$/i, ""), originalFilename: file.name, fileSize: file.size, pageCount: pdf.numPages, createdAt: now, updatedAt: now, lastOpenedAt: now, libraryOrder: index, treatAsTextbook: folderImportTreatAsTextbook, folderId: folder.id, file: data });
          await pdf.destroy();
        } catch { failed++; }
      }
